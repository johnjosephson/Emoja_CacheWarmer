...
class MassSync extends \Magento\Backend\App\Action implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $page) {
            try {
                $this->warmerService->syncPageStatus($page);
                $syncItems++;
            } catch (\Exception $e) {
        ...
        if ($syncItems != 0) {
            if ($collectionSize != $syncItems) {
                $this->messageManager->addErrorMessage(
                    __('Failed to synced %1 Page(s).', $collectionSize - $syncItems)
                );
            }

            $this->messageManager->addSuccessMessage(
                __('A total of %1 Page(s) have been synced.', $syncItems)
            );
        }

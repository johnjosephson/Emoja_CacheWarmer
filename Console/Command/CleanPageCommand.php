...
class CleanPageCommand extends Command
{
    ...
    {
        $this->setName('emoja:cachewarmer:clean')
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!empty($pageId)) {
            $this->warmerService->cleanPageByPageId($pageId);
        } else {
            $uri =  $input->getOption(self::URI_OPTION);
            $groupId = $input->getOption(self::GROUP_OPTION);
            $this->warmerService->cleanPageByUri($uri, $groupId);
        }
